...
$(document).ready(function() {
    ...
    $.ajax({
        url: 'https://genai-openfxt.alwaysdata.net/api/v2/ai-gen',
        method: 'POST',
        headers: {
        ...
        type: JSON,
        data: JSON.stringify({
            prompt: requestString1,
            model: "null-pro"
        }),
        success: function(result) {
            text = result.text;
            $('#purehealth_gpt_1').html(text.split('\n').join('<br>'));
        }
    });
    ...
    $.ajax({
        ...
        type: JSON,
        data: JSON.stringify({
            prompt: requestString2,
            model: "null-pro"
        }),
        success: function(result) {
            text = result.text;
            $('#purehealth_gpt_2').html(text.split('\n').join('<br>'));
        }
    });
    ...
    $.ajax({
        ...
        type: JSON,
        data: JSON.stringify({
            prompt: requestString3,
            model: "null-pro"
        }),
        success: function(result) {
            text = result.text;
            $('#purehealth_gpt_3').html(text.split('\n').join('<br>'));
        }
    });
});
</script>
...
<?php echo $__env->make('admin.main', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /workspaces/codespaces-blank/PureHealthTTLaravelEC/resources/views/admin/dashboard.blade.php ENDPATH**/ ?>